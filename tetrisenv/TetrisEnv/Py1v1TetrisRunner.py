...
class Py1v1TetrisRunner:
    ...
    def collect_trajectory(
        ...
        for t in step_iter:
            ...
            # --- Player 1 action (training policy) ---
            ...
            matches = tf.reduce_all(
                tf.equal(p1_key_sequence[:, None, :], valid_sequences), axis=-1
            )
            action_index = tf.argmax(tf.cast(matches, tf.int64), axis=-1)

            # --- Player 2 action (opponent policy, no data collected) ---
            if self._pathfinding:
            ...
            all_actions = all_actions.write(t, p1_key_sequence)
