...
class Pretrainer():
    ...
    def _load_dataset(self, batch_size: int | None=1024) -> tf.data.Dataset:
        ...
        if not os.path.exists('../tetris_expert_dataset'):
            if input("Dataset not found. Generate? (y/n)") == 'y':
                ...
                raise FileNotFoundError("Dataset not found and user did not want to generate it.")
        else:
            try:
                dataset = tf.data.Dataset.load('../tetris_expert_dataset')
    ...
    def _train_step(self, model: keras.Model, batch) -> tuple[float, float]:
        ...
        # Compute accuracy
        accuracy = tf.reduce_mean(tf.cast(tf.argmax(logits, axis=-1, output_type=tf.int64) == target_action, tf.float32))
...
def main():
    ...
    model.build(input_shape=[(None, 24, 10, 1),
                             (None, queue_size + 2)])
