...
parser = argparse.ArgumentParser(
    description="Tetris Demo with configurable garbage parameters"
)
parser.add_argument(
    "--left",
    nargs="+",
    default=[""],
    help="checkpoint path for left model optionally followed by pathfinding bool (true/false)",
)
parser.add_argument(
    "--right",
    nargs="+",
    default=[""],
    help="checkpoint path for right model optionally followed by pathfinding bool (true/false)",
)
...
def parse_args(arg_list):
    ...
    pathfinding = True
    if len(arg_list) > 1:
        pathfinding = arg_list[1].lower() == "true"
    return path, pathfinding


left_path, left_pathfinding = parse_args(args.left)
right_path, right_pathfinding = parse_args(args.right)
